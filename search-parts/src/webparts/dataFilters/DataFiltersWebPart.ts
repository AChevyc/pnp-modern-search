...
import * as webPartStrings from 'DataFiltersWebPartStrings';
import * as commonStrings from 'CommonStrings';
import DataFilters from './components/DataFiltersContainer';
import { IDataFiltersContainerProps } from './components/IDataFiltersContainerProps';
import IDataFiltersWebPartProps from './IDataFiltersWebPartProps';
...
export default class DataFiltersWebPart extends BaseWebPart<IDataFiltersWebPartProps> implements IDynamicDataCallables {
  ...
  protected renderCompleted(): void {
    ...
    if (this.templateContentToDisplay && this.properties.filtersConfiguration.length > 0) {
      ...
      // Case when no data visualizer WP is connected and we have 'Static' filters configured.
      ...
      renderRootElement = React.createElement(
        DataFilters,
        {
          ...
          onUpdateFilters: (updatedFilters: IDataFilter[]) => {
            ...
            this.context.dynamicDataSourceManager.notifyPropertyChanged(ComponentType.DataFilters);
          },
  ...
  private getConnectionOptions(): IPropertyPaneField<any>[] {
    ...
    const sourceOptions = this.dynamicDataService.getAvailableDataSourcesByType(ComponentType.DataVisualizer).map(source => {
      return {
        text: source.text,
        key: source.key
      } as IComboBoxOption;
  ...
  private ensureDataSourcesConnection() {
    ...
    // Data visualizer Web Part data sources
